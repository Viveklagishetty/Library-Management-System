...
async function loadMyBooks() {
    try {
        ...
        const tbody = document.getElementById("myBooksTableBody") || document.getElementById("booksTableBody");
        if (!tbody) return;
...
async function loadMyBorrows() {
    try {
        ...
        document.getElementById("myTotalHistory").innerText = Array.isArray(history) ? history.length : 0;
        document.getElementById("myActiveBorrows").innerText = Array.isArray(history)
...
async function loadMyFines() {
    try {
        ...
        document.getElementById("myUnpaidFines").innerText = `₹${unpaidTotal.toFixed(2)}`;
    } catch (error) {
...
async function payFineFromWallet(fineId) {
    ...
    try {
        const response = await fetch(`/ebalance/pay-fine/${fineId}`, { method: "POST" });
        const result = await response.json();
...
async function loadMyWallet() {
    try {
        ...
        document.getElementById("myWalletBalance").innerText = `₹${Number(data.balance || 0).toFixed(2)}`;
        document.getElementById("walletBalanceText").innerText = `₹${Number(data.balance || 0).toFixed(2)}`;
