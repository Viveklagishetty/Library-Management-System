...
async function loadTotalBooks() {
  try {
    const response = await fetch(DASH_BOOK_API);

    const result = await response.json();

    if (result.data && document.getElementById("totalBooks")) {
      document.getElementById("totalBooks").innerText = result.data.length;
    }
  } catch (error) {
    console.log(error);
  }
...
async function loadTotalMembers() {
  try {
    const response = await fetch(DASH_MEMBER_API);
...
async function loadActiveBorrows() {
  try {
    const response = await fetch(DASH_ACTIVE_BORROW_API);
...
async function loadOverdueBooks() {
  try {
    const response = await fetch(DASH_OVERDUE_API);

    const result = await response.json();

    if (document.getElementById("overdueBooks")) {
      document.getElementById("overdueBooks").innerText = (result || []).length;
    }
  } catch (error) {
    console.log(error);
  }
...
async function loadTotalFineCollected() {
  try {
    ...
    const response = await fetch(DASH_FINES_API);
    ...
    totalFineElement.innerText = `₹${totalPaid.toFixed(2)}`;
  } catch (error) {
    console.log(error);
  }
}
